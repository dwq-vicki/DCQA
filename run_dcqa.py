...
def eval(model, test_examples, tokenizer, eval_batch_size, choice_num, max_len, max_len_gen, external_sent_num):
    ...
    for step in step_trange:
        ...
        q_ids, q_mask, qo_ids, qo_mask, clue_ids, answers, output_clue = get_input_feature(batch_example,
                                                                                           ...
                                                                                           args.choice_num,
                                                                                           external_sent_num)
        scores, output_sequences, sq_weight, oq_weight = model(q_ids, q_mask, qo_ids, qo_mask, choice_num)

        sq_weight = sq_weight.cpu().detach().tolist()
        oq_weight = oq_weight.cpu().detach().tolist()
        s_w += sq_weight
        o_w += oq_weight
    ...
    return count_right / count, rouge_score, results, sources, s_w, o_w
...
if __name__ == '__main__':
    ...
    tokenizer = T5Tokenizer.from_pretrained(args.model_path, model_max_length=args.max_len)
    model = DCQA(args.model_path, args.num_hidden_layers, args.alpha, args.beta)
    ...
    best_dev_acc, _, _, _, _, _ = eval(model, dev_examples, tokenizer, args.eval_batch_size, args.choice_num, args.max_len,
    ...
    for epoch in range(args.epoch_num):
        ...
        dev_acc, dev_rouge_score, results_dev, dev_sources, dev_sq_weight, dev_oq_weight = eval(model, dev_examples, tokenizer, args.eval_batch_size,
        ...
        if dev_acc > best_dev_acc:
            ...
            save_dataset(path_save_result + '/dev_gen.csv', dev_sources)
            save_weight(path_save_result + '/dev_sq.csv', dev_sq_weight)
            save_weight(path_save_result + '/dev_oq.csv', dev_oq_weight)
            early_stop = 0
            test_acc, test_rouge_score, results_test, test_sources, test_sq_weight, test_oq_weight = eval(model, test_examples, tokenizer, args.eval_batch_size,
                                                            args.choice_num, args.max_len, args.max_len_gen,
                                                            args.external_sent_num)
            save_dataset(path_save_result + '/test.csv', results_test)
            save_dataset(path_save_result + '/test_gen.csv', test_sources)
            save_weight(path_save_result + '/test_sq.csv', test_sq_weight)
            save_weight(path_save_result + '/test_oq.csv', test_oq_weight)
            ...
            with open('accuracy' + str(epoch), 'w', encoding='utf-8') as f:
